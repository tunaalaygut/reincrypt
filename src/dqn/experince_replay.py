...
class Memory:
    ...
    def get_batch(self, target_network, is_training, batch_size, num_actions, gamma):
        ...
        r_idxs = random.sample(range(len(self.current_state)), batch_size)
        
        for k in range(batch_size):
            input_kth = self.current_state[r_idxs[k]]
            action_kth = self.current_action[r_idxs[k]]

            q_act_values = target_network.q_value(self.next_state[r_idxs[k]], is_training)
            next_qs = q_act_values[0]

            target_kth = np.zeros(num_actions)
            target_kth[np.argmax(action_kth)] = self.reward[r_idxs[k]] + gamma * next_qs[0][tf.argmax(next_qs[0])]
