...
def __validate_neutralized_portfolio(vit: ViT, X, y):
    ...
    for t in range(days-1):
        ...
        for c in range(num_currencies):
            ...
            avg_daily_return[t] = np.round(avg_daily_return[t] + cur_reward[c],
                                           8)
            position_change = np.round(position_change + abs(cur_alpha[c] - prev_alpha[c]), 8)
            prev_alpha[c] = cur_alpha[c]
            
    for t in range(days):
        cumulative_asset = np.round(cumulative_asset + (cumulative_asset * avg_daily_return[t] * 0.01), 8)
