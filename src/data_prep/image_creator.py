...
parser.add_argument("-1d", "--one-d", action="store_true",
                    help="Whether to create a 1D rimg")
...
def main():
    ...
    for input_file in os.listdir(INPUT_DIR):
        try:
            ...
            if ONE_D:
                seq_len = IMAGE_SIZE
                tis = np.zeros((len(df), IMAGE_SIZE))
                for i in range(seq_len, len(df) - seq_len):
                    tis[i] = df.Close[i-seq_len:i].values
                images = np.zeros((len(df), IMAGE_SIZE))
